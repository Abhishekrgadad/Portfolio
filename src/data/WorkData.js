export const WorkData = [
	{
		title: "Drowsiness Detection using Opencv-python",
		desc: "An innovative way of Alerting the People who feels drowsy during Driving or in any emergency cases.",
		tech: ["OpenCv-python", "Dlib packages", "python Libraries"],
		gitlink: "//github.com/Abhishekrgadad/Drowsiness_Detection-using-openCV-python",
		site: "//github.com/Abhishekrgadad/Drowsiness_Detection-using-openCV-python/",
	},
	{
		title: "Tourist Places around DK",
		desc: "its an app for mapping out the places with maps around the DK which people can able to cover from start to end include all places with hotels and beaches",
		tech: ["Html", "css", "javascript", "mongodb", "Map api"],
		gitlink: "//github.com/Abhishekrgadad",
		site: "//github.com/Abhishekrgadad",
	},

	// {
	// 	title: "Todo App",
	// 	desc: "A todo list web app",
	// 	tech: ["React.js", "Css", "PWAs"],
	// 	gitlink: "//www.github.com/Pappyjay23/todo-list",
	// 	site: "//todo-list-v0.netlify.app",
	// },

	{
		title: "Netflix Clone",
		desc: "A movie app using the netflix UI with a touch of creativity.",
		tech: ["Html", "Css", "Javascript"],
		gitlink: "//github.com/Abhishekrgadad/Netflix-Clone",
		site: "//github.com/Abhishekrgadad/Netflix-Clone",
	},

	// {
	// 	title: "Budget App",
	// 	desc: "A budget web app",
	// 	tech: ["React.js", "Css"],
	// 	gitlink: "//www.github.com/Pappyjay23/budget-app",
	// 	site: "//budget-app-react-0.netlify.app",
	// },

	// {
	// 	title: "Digitaly",
	// 	desc: "A website for a digital marketing agency",
	// 	tech: ["React.js", "TailwindCss", "Framer Motion"],
	// 	gitlink: "//github.com/Pappyjay23/Digitaly",
	// 	site: "//digitaly.netlify.app/",
	// },
];
